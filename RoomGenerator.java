...
public class RoomGenerator {
    ...
    private ArrayList<Room> seenList = new ArrayList<Room>();
    ...
    public void drawMiniRoom(Room room, Player player) {
        double smallc = .1;
        int relativePlayerx = 50;
        int relativePlayery = -150;
        double x1 = relativePlayerx + room.getx1()*smallc;
        double x2 = room.getx2()*smallc;
        double y1 = relativePlayery + room.gety1()*smallc;
        double y2 = room.gety2()*smallc;
        
        StdDraw.setPenColor(StdDraw.BOOK_LIGHT_BLUE);
        StdDraw.filledRectangle(player.getx()+x1,player.gety()+y1,x2,y2);
        StdDraw.setPenColor(StdDraw.BLUE);
        StdDraw.rectangle(player.getx()+x1,player.gety()+y1,x2,y2);
    }
    ...
    public ArrayList<Room> getrSeenList() {
        return this.seenList;
    }
    
    public ArrayList<Room> updaterSeenList(Player player) {
        for (int i=0;i<this.rooms.size();i++) {
            if (this.rooms.get(i).houses(player) && !this.seenList.contains(this.rooms.get(i))) {
                this.seenList.add(this.rooms.get(i));
            }
        }
        return this.seenList;
    }
