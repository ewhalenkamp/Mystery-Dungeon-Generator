...
public class CorridorGenerator {
    ...
    private ArrayList<Corridor> seenList = new ArrayList<Corridor>();
    private boolean noCorridors = false;
    ...
    public ArrayList<Corridor> createCorridors() {
                            ...
                            if (potentialRoom.getxFirst() - currentRoom.getxSecond() > currentRoom.getxFirst() - potentialRoom.getxSecond()) {
                                ...
                                if (xc1List.size() != 0) {
                                    ...
                                    if (!doesIntersect(xc1,yc1+12.5,xc2,yc2+12.5)) {
                                        ...
                                        currentRoom.cclAdd(potentialRoom);
                                        Corridor corridor = new Corridor(xc1+12.5,yc1+12.5,xc2+12.5,yc2+12.5);
                                        corridorList.add(corridor);
                                        currentRoom.corridorListAdd(corridor);
                                        potentialRoom.corridorListAdd(corridor);
                                    }
                                    xc1List.clear();
                                    xc2List.clear();
                                    yc1List.clear();
        ...
        for (int i=0;i<roomList.size();i++) {
            Room currentRoom = this.roomList.get(i);
            if (currentRoom.getccl().isEmpty()) {
                this.noCorridors = true;
            }
        }
        return corridorList;
    }
    ...
    public void drawMiniCorridorLines(Corridor corridor, Player player) {
        double smallc = .1;
        int relativePlayerx = 50;
        int relativePlayery = -150;
        
        double xc1 = player.getx()+(relativePlayerx + corridor.getxc1()*smallc);
        double xc2 = player.getx()+(relativePlayerx + corridor.getxc2()*smallc);
        double yc1 = player.gety()+(relativePlayery + corridor.getyc1()*smallc);
        double yc2 = player.gety()+(relativePlayery + corridor.getyc2()*smallc);
        StdDraw.setPenColor(StdDraw.BLUE);
        //to-do: change corridor rect width to match pen size
        if (xc1 == xc2) {
            StdDraw.line(xc1+12.5*smallc,yc1-12.5*smallc,xc2+12.5*smallc,yc2-12.5*smallc);
            StdDraw.line(xc1-12.5*smallc,yc1-12.5*smallc,xc2-12.5*smallc,yc2-12.5*smallc);
        }
        else if (yc1 == yc2) {
            StdDraw.line(xc1-12.5*smallc,yc1+12.5*smallc,xc2-12.5*smallc,yc2+12.5*smallc);
    ...
    public void drawMiniCorridorRects(Corridor corridor, Player player) {
        double smallc = .1;
        int relativePlayerx = 50;
        int relativePlayery = -150;
        ...
        //to-do: change corridor rect width to match pen size
        if (xc1 == xc2) {
            StdDraw.filledRectangle(xc1,yc1+(yc2-yc1)/2-12.5*smallc,10.7*smallc,(yc2-yc1)/2+5*smallc);
        }
        else if (yc1 == yc2) {
            StdDraw.filledRectangle(xc1+(xc2-xc1)/2-12.5*smallc,yc1,(xc2-xc1)/2+5*smallc,10.7*smallc);
        }
    }
    
    public ArrayList<Corridor> getcSeenList() {
        return this.seenList;
    }
    
    public ArrayList<Corridor> updatecSeenList(Player player) {
        for (int i=0;i<this.corridorList.size();i++) {
            if (this.corridorList.get(i).houses(player) && !this.seenList.contains(this.corridorList.get(i))) {
                this.seenList.add(corridorList.get(i));
            }
        }
        return this.seenList;
    }
